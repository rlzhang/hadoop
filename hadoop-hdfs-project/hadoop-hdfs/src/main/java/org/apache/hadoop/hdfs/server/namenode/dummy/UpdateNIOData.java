...
public class UpdateNIOData extends Thread {
  ...
  private static Map<String, ConcurrentHashMap<Integer, MapRequest>> serversMap =
  ...
  public UpdateNIOData(MapRequest[] request, String hostName, INodeDirectory parent, long listSize, Connection connection) {
  ...
  public void run() {
    ...
    System.out.println("Server received handleMapRequestArray size is " + map.size());
    this.check();
  }
  ...
  public void updateBlocksMap(INodeFile file) {
    ...
    if (blocks != null) {
      final BlockManager bm = NameNodeDummy.getNameNodeDummyInstance().getBlockManager();
  ...
  private void check() {
    ...
    synchronized(obj) {
      if (listSize == map.size()) {
        ...
        if (NameNodeDummy.DEBUG)
          System.out.println("After merged: inode = " + inode.getFullPathName());
  ...
  private void updateQuota() {
    ...
    if(NameNodeDummy.DEBUG)
      System.out.println(NameNodeDummy.getNameNodeDummyInstance().printNSInfo(INodeServer.root, 0, 10).replaceAll("&nbsp;", " ").replace("<br/>", "\n"));
